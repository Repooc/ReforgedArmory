...
function module:InspectFrame_OnShow()
	...
	if isSkinned then
		...
		_G.InspectSecondaryHandSlot:ClearAllPoints()
		_G.InspectSecondaryHandSlot:SetPoint('BOTTOM', _G.PaperDollItemsFrame, 'BOTTOM', 0, 16)
		_G.InspectMainHandSlot:ClearAllPoints()
		_G.InspectMainHandSlot:SetPoint('TOPRIGHT', _G.InspectSecondaryHandSlot, 'TOPLEFT', -5, 0)
	end
...
-- function RA:Init(event, addon)
-- 	if event == 'ADDON_LOADED' and (RA.Initialized or IsAddOnLoaded(AddOnName)) then
-- 		if addon == AddOnName then
-- 			RA.Initialized = true
-- 			-- RA:BuildProfile()

-- 			-- RA:SetupDetails()
-- 			-- RA:SetupBlizzardFrames() --* Setup class icon icon for frames
-- 			-- RA:SetupSUF()
...
-- 		-- RA:BuildOptions()
-- 		-- RA:RegisterEvent('PLAYER_ENTERING_WORLD', function() print(L["LOGIN_MSG"]) end)
-- 		-- RA:SecureHook('UnitFramePortrait_Update', 'UnitFramePortrait_Update')
-- 	end
-- end

-- RA:RegisterEvent('ADDON_LOADED', 'Init')
-- RA:RegisterEvent('PLAYER_LOGIN', 'Init')
