...
local MAJOR, MINOR = 'LibAceConfigHelper-Reforged', 11
...
function ACH:FontFlags(name, desc, order, width, get, set, disabled, hidden)
	local optionTable = { type = 'select', name = name, desc = desc, order = order, get = get, set = set, disabled = disabled, hidden = hidden, values = ACH.FontValues }
